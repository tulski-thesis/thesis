...
\todo{W tekście niżej odmieniam słowo backend (backendem, backendu itp.) - nie wiem czy to poprawne}

Panel administracyjny to kolejny element rozbudowanego ekosystemu Medusa, który jest dostarczany w formie wtyczki (ang. \emph{plugin}).
...
Wtyczka panelu administracyjnego, skonfigurowana w plikach serwera backend (zob. listing~\ref{lst:medusajs-admin-plugin}), może być uruchomiona równolegle z backendem, lub jako niezależny serwer.
...
Panel administracyjny jest budowany do postaci statycznych plików HTML, CSS i JavaScript, które następnie są hostowane przez serwer NGINX (zob. listing~\ref{lst:medusajs-admin-dockerfile}).

\begin{listing}[H]
    \inputminted[xleftmargin=20pt,linenos]{docker}{code/Dockerfile.admin}
    \caption{Plik Dockerfile panelu administracyjnego}
    \label{lst:medusajs-admin-dockerfile}
\end{listing}

Panel administracyjny w środowisku Kubernetes funkcjonuje jako osobna jednostka wdrożeniowa, niezależna od backendu.
...
\begin{figure}[H]
    ...
    \caption{Panel administracyjny w środowisku Kubernetes}
