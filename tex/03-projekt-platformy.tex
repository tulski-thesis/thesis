...
\textquote[Strona Medusa{\cite{medusajs-homepage}}]{
    Tysiące programistów korzysta z otwartoźródłowych modułów i narzędzi handlowych Medusa, aby tworzyć bogate, niezawodne i wydajne aplikacje handlowe, nie wynajdując od nowa podstawowej logiki handlu.
}
...
\todo{Jak się tłumaczy backend? Czy tekst niżej jest potrzebny? Chyba każdy rozumie czym jest backend. Czy użyty język nie jest zbyt luźny?}
...
W tym celu zastosowano \mintinline{text}{medusa-fulfillment-manual}  i \mintinline{text}{medusa-payment-manual}.
...
W tym celu stworzono plik \mintinline{text}{Dockerfile}, który opisuje jak definiuje sposób budowania oraz uruchomienia aplikacji.
...
\todo{Zastanawiam się czy ta sekcja jest potrzebna. W sumie to ten admin panel zrobiłem tylko dlatego, że to było mega proste. Nie musiałem tego robić}
...
\todo{Baza PostgreSQL z helm charta}
